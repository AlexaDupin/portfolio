...
export function ExpandCard({ number, eyebrow, title, summary, children }: ExpandCardProps) {
  ...
  return (
    <div
      className={cn(
        "bg-card rounded-2xl border p-7 transition-[border-color,transform] duration-200",
        open ? "border-green/20" : "border-green/10 hover:border-green/30 hover:scale-[1.02]"
      )}
    >
      <button
        onClick={() => setOpen((o) => !o)}
      ...
      >
        ...
        <h3 className="text-green mt-3 font-serif text-2xl leading-snug">{title}</h3>
        <p className="text-green mt-3 font-sans text-base leading-relaxed">{summary}</p>
        <p className="text-mustard mt-5 font-sans text-base">
