...
export function StatusPill({ label, variant, className }: StatusPillProps) {
  ...
  if (variant === "filled") {
    return <span className={cn(base, "bg-mustard text-green", className)}>{label}</span>;
  }

  if (variant === "outlined") {
    return <span className={cn(base, "border-green text-green border", className)}>{label}</span>;
  }

  return (
    <span className={cn(base, "bg-green text-mustard", className)}>
